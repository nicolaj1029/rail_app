...
class FerryLandingController extends AppController
{
    /** Render the canonical public FERRY landing. */
    public function modern(): Response
    {
        $this->viewBuilder()->setLayout('landing_fullbleed');
        $this->prepareFerryLandingView('/faerge-ny');
    ...
    private function prepareFerryLandingView(string $brandHref): void
    {
        ...
        $quickLinks = [
            'ongoing' => Router::url($this->localizedLandingUrl('/flow/ferry/ongoing' . $flowQuery, $lang), true),
            'completed' => Router::url($this->localizedLandingUrl('/flow/ferry/completed' . $flowQuery, $lang), true),
            'beforeStart' => Router::url($this->localizedLandingUrl('/flow/ferry/before_start' . $flowQuery, $lang), true),
        ];
